...
9. **S-009** — Check rendered route discovery on a disposable generated site's public profile, not just source or build output. Before browsing, record the approved route inventory, expected visible paths between top-level routes, and any required CTA destination; do not infer these from hidden DOM links. Start the site with `npm run dev` (or serve the public `dist/` statically) on loopback and open it in an owned browser window. Record the exact startup command, port and local origin for reproduction.
   - Visit every inventoried top-level route at 360×800, 390×844, and one recorded desktop viewport (e.g. 1280×900). For each route/viewport, observe visible navigation or an operable disclosure leading to every required top-level route; check visibility, activation and destination of every expected navigation link (including the home/brand path). A reachable route alone does not prove discovery. Navigation is `not_applicable` only if the approved route inventory explicitly has no cross-route navigation requirement; record why, rather than inferring this from a missing header.
   - If a CTA is required, inspect its visible, unclipped label and destination, then test click/hit target or keyboard activation. Mark CTA `not_applicable` only when the approved inventory has no CTA; missing required navigation is `fail`, not `not_applicable`.
   - Tab through the brand, navigation and applicable CTA; record focus order, visible focus indication and whether activation reaches the expected destination. Compare `document.documentElement.scrollWidth` with `window.innerWidth`; inspect any excess to distinguish unintended page overflow from an intentional scroll region. These observations do not certify WCAG compliance.
   - Record one sanitized row per route/viewport: viewport, route, visible path/link targets, CTA result or `not_applicable`, focus/activation result, scroll/inner widths, `pass`/`fail`, and screenshot or equivalent rendered-observation locator. Mark unavailable browser checks `not_run` with reason, never `pass` from `npm run verify` or `npm run build`. Evidence: new run-specific verification record; do not reuse historical `VR-ACCEPT-003` as proof of this check.
